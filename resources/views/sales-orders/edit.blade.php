<x-app-layout>
    <x-slot name="header">Edit SO #{{ $salesOrder->id_so }}</x-slot>

    <div class="max-w-4xl">
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            <form action="{{ route('sales-orders.update', $salesOrder) }}" method="POST" x-data="{
                items: {{ $salesOrder->details->map(fn($d) => ['sku' => $d->sku, 'nama_barang' => $d->barang->nama_barang ?? '', 'qty' => $d->qty, 'harga' => $d->harga])->toJson() }},
                ...
                <div class="grid grid-cols-2 gap-4 mb-5">
                    <div>
                        ...
                        <select name="id_customer" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                            ...
                            <option value="{{ $c->id_customer }}" @selected($salesOrder->id_customer == $c->id_customer)>{{ $c->nama_customer }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Date</label>
                        <input type="date" name="tanggal" value="{{ old('tanggal', $salesOrder->tanggal->format('Y-m-d')) }}" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                    </div>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <select name="status" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                        ...
                        <option value="dikirim" @selected($salesOrder->status == 'dikirim')>Shipped</option>
                        <option value="selesai" @selected($salesOrder->status == 'selesai')>Completed</option>
                    </select>
                </div>
                        ...
                        <div class="flex gap-3 items-center mb-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <div class="flex-1">
                                <select :name="'items[' + index + '][sku]'" x-model="item.sku" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                    ...
                                    <option value="{{ $product->sku }}" x-bind:selected="item.sku == '{{ $product->sku }}'">{{ $product->sku }} — {{ $product->nama_barang }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="w-20">
