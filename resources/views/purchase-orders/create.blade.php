<x-app-layout>
    ...
    <div class="max-w-4xl">
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            <form action="{{ route('purchase-orders.store') }}" method="POST" x-data="{
                purchaseRequestId: '',
                items: [{ sku: '', qty: 1, price: 0 }],
                getPurchaseRequestItems() {
                    const select = document.getElementById('purchase_request_id');
                    return select ? JSON.parse(select.options[select.selectedIndex]?.dataset.items || '[]') : [];
                },
                loadItems() {
                    const items = this.getPurchaseRequestItems();
                    if (items.length) {
                        this.items = items.map(i => ({ sku: i.sku, qty: i.qty, price: 0 }));
                    }
                }
            }">
                @csrf
                ...
                <div class="mb-5">
                    ...
                    <template x-for="(item, index) in items" :key="index">
                        <div class="flex gap-3 items-start mb-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            ...
                            <div class="w-28 pt-2.5 text-sm text-gray-500 dark:text-gray-400" x-text="'$' + (item.qty * item.price).toLocaleString()"></div>
