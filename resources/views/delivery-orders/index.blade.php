            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr class="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">#{{ $deliveryOrder->id_do }}</td>
                        <td class="px-6 py-4 dark:text-gray-300">#{{ $deliveryOrder->so->id_so }}</td>
                        <td class="px-6 py-4 dark:text-gray-300">{{ $deliveryOrder->so->customer->nama_customer }}</td>
                        <td class="px-6 py-4 dark:text-gray-300">{{ $deliveryOrder->tanggal->format('d/m/Y') }}</td>
                        <td class="px-6 py-4 dark:text-gray-300">{{ $deliveryOrder->details->count() }} items</td>
                        <td class="px-6 py-4 flex gap-2">
                            ...
                            <x-delete-modal :route="route('delivery-orders.destroy', $deliveryOrder)" label="DO #{{ $deliveryOrder->id_do }}" />
                            @endcan
                        </td>
                    </tr>
                    @empty
