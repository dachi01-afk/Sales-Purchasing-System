<x-app-layout>
    <x-slot name="header">Edit Invoice #{{ $invoicePurchasing->id_invoice_purchasing }}</x-slot>

    <div class="max-w-4xl">
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            <form action="{{ route('invoice-purchasing.update', $invoicePurchasing) }}" method="POST" x-data="{
                poId: {{ $invoicePurchasing->id_po }},
                items: {{ $invoicePurchasing->details->map(fn($d) => ['sku' => $d->sku, 'nama_barang' => $d->barang->nama_barang ?? '', 'qty' => $d->qty, 'harga' => $d->harga])->toJson() }},
                ...
                <div class="grid grid-cols-2 gap-4 mb-5">
                    <div>
                        ...
                        <select name="id_po" x-model="poId" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                            ...
                            @foreach($pos as $po)
                            <option value="{{ $po->id_po }}" @selected($invoicePurchasing->id_po == $po->id_po)>#{{ $po->id_po }} — {{ $po->vendor->nama_vendor }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tanggal</label>
                        <input type="date" name="tanggal" value="{{ old('tanggal', $invoicePurchasing->tanggal->format('Y-m-d')) }}" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                    </div>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <select name="status" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                        <option value="draft" @selected($invoicePurchasing->status == 'draft')>Draft</option>
                        <option value="lunas" @selected($invoicePurchasing->status == 'lunas')>Lunas</option>
                    </select>
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('invoice-purchasing.index') }}" class="text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5">Batal</a>
                </div>
            </form>
        </div>
    </div>
