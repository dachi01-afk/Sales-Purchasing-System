<x-app-layout>
    <x-slot name="header">Edit Purchase Request</x-slot>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            <form action="{{ route('purchase-requests.update', $purchaseRequest) }}" method="POST" x-data="{ items: {{ $purchaseRequest->details->map(fn($d) => ['sku' => $d->sku, 'qty' => $d->qty, 'notes' => $d->notes])->toJson() }} }">
                ...
                <div class="mb-5">
                    ...
                    <input type="date" name="date" value="{{ old('date', $purchaseRequest->date->format('Y-m-d')) }}" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full max-w-xs p-2.5 @error('date') border-red-500 @enderror" required>
                ...
                <div class="mb-5">
                    ...
                    <textarea name="notes" rows="2" class="bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">{{ old('notes', $purchaseRequest->notes) }}</textarea>
                </div>
